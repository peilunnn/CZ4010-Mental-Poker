...
class PlayerConnection(Node):
    """
    This class represents a player in the mental poker game. It handles the network connections
    and the game's protocols for deck preparation, shuffling, and card drawing.
    """
    ...
    def __init__(self, host, port, id=None, callback=None, max_connections=0):
        ...
        Args:
            host (str): The host address of the player.
            port (int): The port number of the player.
            id (str): The unique identifier for the player.
            callback (function): The callback function for node events.
            max_connections (int): The maximum number of connections the node can have.
        """
    ...
    def outbound_node_connected(self, connected_node):
        """
        Handle new outbound connections to other nodes.

        Args:
            connected_node (Node): The node that has been connected to.
        """
        print(f"{self.id}: outbound_node_connected: {connected_node.id}")

    def inbound_node_connected(self, connected_node):
        """
        Handle new inbound connections from other nodes.

        Args:
            connected_node (Node): The node that has connected.
        """
        print(f"{self.id}: inbound_node_connected: {connected_node.id}")

    def inbound_node_disconnected(self, connected_node):
        """
        Handle the disconnection of an inbound node.

        Args:
            connected_node (Node): The node that has disconnected.
        """
        print(f"{self.id}: inbound_node_disconnected: {connected_node.id}")

    def outbound_node_disconnected(self, connected_node):
        """
        Handle the disconnection of an outbound node.
    ...
    def node_message(self, connected_node, data):
        """
        Handle messages received from connected nodes.

        Args:
            connected_node (Node): The node that sent the message.
            data (dict): The data received with the message.
        """
        ...
        if msg_type == "TEST":
            ...
            for i in range(0, 53):
                ...
                card_prep_msg.append(
                    [[g.x, g.y], [gl.x, gl.y], [h.x, h.y], [hl.x, hl.y], r, t]
                )
                self.deck.prepare_card(hl, i)

            self.send_to_nodes({"type": "CARD_PREP", "cards": card_prep_msg})
            ...
            for i in range(0, 53):
                ...
                gl = ec.Point(
                    self.curve, data["cards"][i][1][0], data["cards"][i][1][1]
                )
                h = ec.Point(self.curve, data["cards"][i][2][0], data["cards"][i][2][1])
                hl = ec.Point(
                    self.curve, data["cards"][i][3][0], data["cards"][i][3][1]
                )
                r = data["cards"][i][4]
                t = data["cards"][i][5]
                ...
                if not verify_nizk_dleq(g, gl, h, hl, r, t):
                    print(
                        f"{self.id}: Detected cheating from: {connected_node.id}, I should quit."
                    )
                else:
                    print(
                        f"{self.id}: NIZK from {connected_node.id} verified... preparing card {i}"
                    )
                    self.deck.prepare_card(hl, i)
            ...
            (self.secret, self.permutation, self.shuffled_deck, m) = gen_nizk_shuffle(
                self.deck
            )
            ...
            # Set new deck state
            self.peer_shuffle_state[self.id] = (
                self.deck.cards[0],
                self.shuffled_deck.cards[0],
            )
            ...
            if verified:
                ...
                self.peer_shuffle_state[connected_node.id] = (
                    self.deck.cards[0],
                    self.shuffled_deck.cards[0],
                )
                self.deck = self.shuffled_deck
            ...
            for idx in idxs:
                ...
                (r, t) = gen_nizk_dleq(
                    ...
                    c,
                    self.deck.cards[idx],
                    self.peer_shuffle_state[self.id][0],
                    self.peer_shuffle_state[self.id][1],
                    self.secret,
                )
                cs.append([c.x, c.y])
                rs.append(r)
            ...
            print(
                f"{self.id}: Received DRAW_CARDS_RESPONSE msg from: {connected_node.id}"
            )
            idxs = data["idxs"]
            x_inv = ec.mod_inv(self.secret, self.deck.curve.field.n)
            ...
            for i in range(0, len(idxs)):
                ...
                r = data["rs"][i]
                t = data["ts"][i]
                verified = verify_nizk_dleq(
                    c,
                    self.deck.cards[idxs[i]],
                    self.peer_shuffle_state[connected_node.id][0],
                    self.peer_shuffle_state[connected_node.id][1],
                    r,
                    t,
                )
            ...
            self.send_to_nodes(
                {"type": "REVEAL_CARDS", "cards": card_list, "rs": rs, "ts": ts}
            )
            ...
            for i in range(0, len(claimed_cards)):
                ...
                verified = verify_nizk_dleq(
                    c,
                    self.peer_hand_state[connected_node.id][i],
                    self.peer_shuffle_state[connected_node.id][0],
                    self.peer_shuffle_state[connected_node.id][1],
                    r,
                    t,
                ...
                print(
                    f"{self.id}: card {i} verified from {connected_node.id}? {verified}"
                )
            ...
            print(
                f"Received message of unknown type {msg_type} from node: {connected_node.id}"
            )

    def node_disconnect_with_outbound_node(self, connected_node):
        """
        Handle the request to disconnect from an outbound node.

        Args:
            connected_node (Node): The node to disconnect from.
        """
        print("node wants to disconnect with other outbound node: " + connected_node.id)

    def node_request_to_stop(self):
        """
        Handle the request for the node to stop.
        """
